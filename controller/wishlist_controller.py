...
from data_adapter.wishlist import list_wishlists, get_wishlist, create_new_wishlist, update_wishlist as update_wishlist_db, delete_wishlist as delete_wishlist_db
...
def return_wishlist_by_uuid(wishlist_uuid: str, db:Session=Depends(get_db)):
    wishlist = get_wishlist(wishlist_uuid,db)
    if wishlist is None:
...
def update_wishlist(wishlist_uuid: str, wishlist:WishlistCreate, db:Session=Depends(get_db)):
    ...
    if response is False:
        raise HTTPException(status_code=http.HTTPStatus.NOT_FOUND, detail=f"wishlist with uuid {wishlist_uuid} does not exist")
    return {"detail": "wishlist successfully updated"}
...
def delete_wishlist(wishlist_uuid: str, db:Session=Depends(get_db)):
    ...
    if response is False:
        return {"detail": f"wishlist with uuid {wishlist_uuid} does not exist"}
    return {"detail": "wishlist successfully deleted"}
